...
// Konstanta ukuran tubuh hexapod
...
extern int Yoff;
extern int Xoff;
    // Offset pada sumbu X (horizontal depan-belakang), menyesuaikan posisi kaki depan/belakang
...
void setupBody() {
    ...
    body[3].awalX = -lebarKecilBody;
    body[4].awalX = -lebarBody;
    body[5].awalX = -lebarKecilBody;

    body[0].awalY = panjangBody;
    body[1].awalY = 0;
    body[2].awalY = -panjangBody;
    body[3].awalY = -panjangBody;
    body[4].awalY = 0;
...
// Fungsi untuk menerapkan semua rotasi (roll, pitch, yaw) dan translasi linear (Xoff, Yoff)
void applyBodyRotation(float rollAngle, float pitchAngle, float yawAngle) {
    ...
    // Aplikasikan rotasi dan translasi ke setiap kaki
    for (int i = 0; i < 6; i++) {
        float tempX, tempY, tempZ;

        // Rotasi dengan matriks roll (sumbu X)
        multiplyMatrixVector(rotX, body[i].awalX, body[i].awalY, body[i].awalZ, tempX, tempY, tempZ);

        // Rotasi dengan matriks pitch (sumbu Y)
        multiplyMatrixVector(rotY, tempX, tempY, tempZ, tempX, tempY, tempZ);

        // Rotasi dengan matriks yaw (sumbu Z)
        multiplyMatrixVector(rotZ, tempX, tempY, tempZ, body[i].posX, body[i].posY, body[i].posZ);

        // Tambahkan translasi linear (Xoff, Yoff)
        body[i].posX += Xoff;
        body[i].posY += Yoff;
    }
}

// Fungsi untuk mengatur translasi linear
void setTranslation(float deltaX, float deltaY) {
    Xoff = deltaX;
    Yoff = deltaY;
}
