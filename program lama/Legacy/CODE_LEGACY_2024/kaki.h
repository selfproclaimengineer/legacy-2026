...
#define femur 80 // Panjang segmen Femur (paha bawah dari sendi pertama ke sendi kedua)
#define tibia 90// Panjang segmen Tibia (betis dari sendi kedua ke ujung kaki)
...
int tinggiLangkah = 100;  // Tinggi langkah kaki saat diangkat dari tanah (dibagi rateKu harus bulat)
...
int Zoff = -120;  // Offset pada sumbu Z (tinggi), menyesuaikan tinggi standar kaki di atas tanah
...
void setKaki() {
  ...
  // Kondisi default (Yoff != -20)
    ...
    // Pengaturan tinggi kaki
    int tinggiValues[6] = {0, 0, 0, 0, 5, 0};
    // Pengaturan bukaan kaki
    int bukaanValues[6] = {0, 0, 10, 0, 10, 0};
...
void setupKaki() {
  ...
  setKaki();
  setGerak();
  bodyKinematics(0, 0.0, 0.0);  // Jika ingin tidak ada rotasi
  syncLeg();
}
...
void passData(int masuk) {
  ...
  if (inputBefore == 0) {
    ...
    setGerak();  
    bodyKinematics(0, 0, 0);  // Jika ingin tidak ada rotasi
